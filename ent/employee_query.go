...
type EmployeeQuery struct {
	...
	withSalaryCalculations *SalaryCalculationQuery
...
// QuerySalaryCalculations chains the current query on the "salary_calculations" edge.
func (eq *EmployeeQuery) QuerySalaryCalculations() *SalaryCalculationQuery {
	query := (&SalaryCalculationClient{config: eq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := eq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (eq *EmployeeQuery) Clone() *EmployeeQuery {
	...
	return &EmployeeQuery{
		...
		withSalaryCalculations: eq.withSalaryCalculations.Clone(),
...
// WithSalaryCalculations tells the query-builder to eager-load the nodes that are connected to
// the "salary_calculations" edge. The optional arguments are used to configure the query builder of the edge.
func (eq *EmployeeQuery) WithSalaryCalculations(opts ...func(*SalaryCalculationQuery)) *EmployeeQuery {
	query := (&SalaryCalculationClient{config: eq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	eq.withSalaryCalculations = query
	return eq
}
...
func (eq *EmployeeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Employee, error) {
	...
	if query := eq.withSalaryCalculations; query != nil {
		if err := eq.loadSalaryCalculations(ctx, query, nodes,
			func(n *Employee) { n.Edges.SalaryCalculations = []*SalaryCalculation{} },
			func(n *Employee, e *SalaryCalculation) {
				n.Edges.SalaryCalculations = append(n.Edges.SalaryCalculations, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (eq *EmployeeQuery) loadSalaryCalculations(ctx context.Context, query *SalaryCalculationQuery, nodes []*Employee, init func(*Employee), assign func(*Employee, *SalaryCalculation)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint64]*Employee)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(salarycalculation.FieldEmployeeID)
	}
	query.Where(predicate.SalaryCalculation(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(employee.SalaryCalculationsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
